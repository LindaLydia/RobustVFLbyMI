...
class VFLDefenceExperimentBase(object):
    ...
    def train_batch_local(self, batch_data_a, batch_data_b, batch_label, net_a, net_b, encoder, model_optimizer, criterion):
        if self.apply_trainable_layer:
    ...
    def train_batch_mid(self, batch_data_a, batch_data_b, batch_label, net_a, net_b, encoder, model_optimizer, criterion):
        model_optimizer = torch.optim.Adam(list(self.active_aggregate_model.parameters()) + list(self.mid_model.parameters()) + list(self.mid_enlarge_model.parameters()), lr=self.lr)
        gt_equal_probability = torch.from_numpy(np.array([1/self.num_classes]*self.num_classes)).to(self.device)
        if self.apply_encoder:
            if self.encoder:
                _, gt_one_hot_label = self.encoder(batch_label)
    ...
    def train(self):
        ...
        for i_epoch in range(self.epochs):
            ...
            for i, (gt_data, gt_label) in enumerate(tqdm_train):
                ...
                if self.apply_mid:
                    if i_epoch < (self.epochs//3) or i_epoch >= ((self.epochs<<1)//3):
                        net_a.train()
                        net_b.train()
                        if self.apply_mid:
                            self.mid_model.eval()
                        ...
                        # print('before batch, gt_one_hot_label:', gt_one_hot_label)
                        # ====== train batch ======
                        loss, train_acc = self.train_batch_local(gt_data_a, gt_data_b, gt_one_hot_label,
                                                    net_a, net_b, self.encoder, model_optimizer, criterion)
                    # elif i_epoch >= ((self.epochs<<1)//3):
                    #     net_a.train()
                    #     net_b.train()
                    #     if self.apply_mid:
                    #         self.mid_model.train()
                    ...
                    #                                 net_a, net_b, self.encoder, model_optimizer, criterion)
                        ...
                        # print('before batch, gt_one_hot_label:', gt_one_hot_label)
                        # ====== train batch ======
                        loss, train_acc = self.train_batch_mid(gt_data_a, gt_data_b, gt_one_hot_label,
                                                    net_a, net_b, self.encoder, model_optimizer, criterion)
                else:
                    net_a.train()
                    net_b.train()
