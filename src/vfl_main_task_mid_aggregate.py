...
class VFLDefenceExperimentBase(object):
    ...
    def train_batch(self, batch_data_list, batch_label, nets, encoder, model_optimizer, criterion):
        ...
        if self.apply_mi:
            ...
            loss = criterion(pred, gt_one_hot_label) + self.mi_loss_lambda * criterion(preds[0], gt_equal_probability) - self.mi_loss_lambda * criterion(pred_a, pred_a)
        ...
        ######################## defense3: mid ############################
            ...
            pred = self.active_aggregate_model([pred_Z, preds[-1]])
            ...
            if (i + 1) % print_every == 0:
                ...
                with torch.no_grad():
                    ...
                    for gt_val_data, gt_val_label in val_loader:
                        ...
                        if self.apply_mid:
                            epsilon = torch.empty((test_logit_list[0].size()[0],test_logit_list[0].size()[1]*BOTTLENECK_SCALE))
                            # # # discrete form of reparameterization
                            # # torch.nn.init.uniform(epsilon) # epsilon is initialized
                            # # epsilon = - torch.log(epsilon + torch.tensor(1e-07))
                            # # epsilon = - torch.log(epsilon + torch.tensor(1e-07)) # prevent if epsilon=0.0
                            ...
                            out = test_logit_list[0]
                            for i in range(len(test_logit_list)-2):
                                out = out.add(test_logit_list[i+1])
                            test_logit_a_double = self.mid_enlarge_model(out)
                            ...
                            test_logit = self.active_aggregate_model([test_logit_Z, test_logit_list[-1]])
