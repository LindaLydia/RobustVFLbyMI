...
def save_defense_data(target_dataset, target_model, exp_dir):
    ...
    for item in os.listdir(exp_dir):
        ...
        if os.path.isdir(sub_dir):
            ...
            # epoch - dataset - model - batch_size - name - \
            # backdoor - amplify_rate - amplify_rate_output - dp_type - dp_strength - \
            # gradient_sparsification - certify - sigma - autoencoder - lba - mid - mid_lambda \
            # seed - use_project_head - random_output - learning_rate - timestamp
            ...
            if defense_model=='none' and discrete_bins in [6,12,18] and str(temps[17])=='True':
                defense_model = 'dsgd'
            if autoencoder_coef == 0.0 and float(temps[15]) != 0.0:
                defense_model = 'mid'
            # else:
            #     autoencoder_coef = 0
    ...
    save_dir = ''
    if 'rvfr' in exp_dir:
        save_dir = './images_missing/rvfr_{}/data/'.format(exp_dir.split('_')[-2])
    else:
        save_dir = './images_missing/{}/data/'.format(exp_dir.split('_')[-2])
    print(save_dir)
    if not os.path.exists(save_dir):
        os.makedirs('./images_missing/{}/data/'.format(exp_dir.split('_')[-2]))
    with open(save_dir+'{}_{}.json'.format(target_dataset, target_model), 'w') as f:
...
def plot_scatter_plot_for_paper(input_file, target_dir, marker=False):
    ...
    for key in data:
        ...
        if defense_method in ['gaussian', 'laplace']:
            ...
            if defense_param not in ['0.1', '0.01', '0.001', '0.0001']:
                continue
        elif defense_method == 'autoencoder':
    ...
    fig, ax = plt.subplots()
    marker_list = ['o', 'v', '^', 'x', '*']
    method_name_dict = {'gaussian': 'DP-G', 'laplace': 'DP-L', 'gradient_sparsification': 'GS', 'certifyFL': 'CFL', 'autoencoder': 'CAE', 'mid': 'MID'}
