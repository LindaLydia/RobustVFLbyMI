...
def transform_to_pred_labels(logits, encoder):
    ...
    # dec_predict_prob = encoder.decoder(encoder.extention(enc_predict_prob))
...
def main():
    ...
    # parser.add_argument('--learning_rate', type=float, default=0.008, help='init learning rate')
    ...
    # parser.add_argument('--model', default='lenet', help='resnet')
    # parser.add_argument('--model', default='simplecnn', help='resnet')
    # parser.add_argument('--model', default='resnet18', help='resnet')
    ...
    if args.apply_discrete_gradients:
        args.name = 'experiment_plot_model_DSGD_50_{}/{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}'.format(
            ...
            args.use_project_head, args.random_output, args.learning_rate, args.apply_discrete_gradients, time.strftime("%Y%m%d-%H%M%S"))
    else:
        args.name = 'experiment_plot_nuswide_50_{}/{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}-{}'.format(
            args.name, args.epochs, args.dataset, args.model, args.batch_size, args.name,args.backdoor, args.amplify_rate,
            args.amplify_rate_output, args.dp_type, args.dp_strength, args.gradient_sparsification, args.certify, args.sigma, args.autoencoder, args.lba, args.seed,
    ...
    if args.dataset == 'mnist':
        ...
        # target_label_index = random.randint(0, 3)
        # target_label_list = [5,3,7]
        # target_label = target_label_list[target_label_index]
    ...
    if apply_encoder == 1:
        ...
        # model_timestamp = '1629921540'  # n_classes 20
        # model_timestamp = '1630264303'  # n_classes 20
        # model_timestamp = '1630132602'  # n_classes 20
        # model_timestamp = '1629765842'  # n_classes 10 no entropy loss
        # model_timestamp = '1630306247'  # n_classes 5 no entropy loss
        # model_timestamp = '1630306162'  # n_classes 5 with entropy loss
        # model_timestamp = '1630308532'  # n_classes 5 with entropy loss
        # model_timestamp = '1630309262'  # n_classes 5 with entropy loss *
        # model_timestamp = '1630310779'  # n_classes 5 with entropy loss
        # model_timestamp = '1630313890'  # n_classes 5 with entropy loss
        # model_timestamp = '1630340681'  # n_classes 5 with entropy loss
        # model_timestamp = '1630378439'  # n_classes 5 with entropy loss
        # model_timestamp = '1630382931'  # n_classes 5 with entropy loss
        # model_timestamp = '1630309262'  # n_classes 5 with entropy loss *
        # model_timestamp = '1630384953'  # n_classes 5 with entropy loss
        # model_timestamp = '1630392815'  # n_classes 5 with entropy loss and negative loss
        # model_timestamp = '1630553955'

        # lambda_2 = '0.1'
        # model_timestamp = '1635176246'
        # lambda_2 = '0.5'
        # model_timestamp = ''
        lambda_2 = args.lba
        ...
        # encoder = AutoEncoder_extend(input_dim=dim, encode_dim=2 + dim * 6).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for step, (trn_X, trn_y) in enumerate(train_loader):
            ...
            if encoder:
                ...
                #     _, tr_target_one_hot = encoder(target_one_hot)
                # else:
                #     _, tr_target_one_hot = get_random_softmax_onehot_label(encoder, device, target_one_hot)
        ...
        with torch.no_grad():
            ...
            # z_up_random = torch.normal(mean_z_up.item(), std_z_up.item(), z_up.shape).float().to(device)

            # # random down output
            # std_z_down, mean_z_down = torch.std_mean(z_down)
            # z_down_random = torch.normal(mean_z_down.item(), std_z_down.item(), z_down.shape).float().to(device)
            ...
            if encoder:
                ...
                # acc = utils.accuracy3(pre_random_up, backdoor_labels)
                ...
                # acc = utils.accuracy3(pre_random_down, backdoor_true_labels)
                ...
                # acc = utils.accuracy3(pre_target_down, backdoor_labels)
                ...
                # prec1 = utils.accuracy(logits_random_up, backdoor_labels, topk=(1,))
                ...
                # prec1 = utils.accuracy(logits_randon_down, backdoor_true_labels, topk=(1,))
                ...
                # prec1 = utils.accuracy(logits_target_down, backdoor_labels, topk=(1,))
        ...
        #            'Backdoor Loss random: {:.4f}, Backdoor Accuracy random: {:.2f}, ' \
        #            'Backdoor Loss random down: {:.4f}, Backdoor Accuracy random down: {:.2f}, ' \
        #            'Backdoor Loss target down: {:.4f}, Backdoor Accuracy target down: {:.2f}\n' \
        #            'up mean: {:.4f}, up std: {:.4f}, ' \
        #            'down mean: {:.4f}, down std: {:.4f}, ' \
        #            'down target mean: {:.4f}, down target std: {:.4f}'
        ...
        #                              losses_backdoor,
        #                              top1_backdoor.item(),
        #                              losses_backdoor_random_up,
        #                              top1_backdoor_random_up.item(),
        #                              losses_backdoor_random_down,
        #                              top1_backdoor_random_down.item(),
        #                              losses_backdoor_target_down,
        #                              top1_backdoor_target_down.item(),
        #                              mean_z_up.item(),
        #                              std_z_up.item(),
        #                              mean_z_down.item(),
        #                              std_z_down.item(),
        #                              mean_z_down_target.item(),
        #                              std_z_down_target.item()
        #                              ))
