...
#
#         layers = []
#         layers.append(block(self.inplanes, planes, stride, downsample))  # 每个blocks的�??一个residual结构保存在layers列表�?�?
#         self.inplanes = planes * block.expansion
#         for i in range(1, blocks):
#             layers.append(block(self.inplanes, planes))  # 该部分是将每个blocks的剩下residual 结构保存在layers列表�?，这样就完成了一个blocks的构造�?
#
#         return nn.Sequential(*layers)
#
#     def forward(self, x):
