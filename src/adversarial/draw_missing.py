...
def plot_scatter_plot_for_paper(input_file, target_dir, marker=False):
    ...
    ax.scatter([none_main],[none_main-none_attack], label='w/o defense', marker='s', color='k', s=150)
...
def plot_scatter_plot_for_paper_smaller(input_file, target_dir, marker=False):
    ...
    #         ax.scatter([0.5],[5.0], label='w/o defense', marker='s', color='k', s=150)
    # ax.scatter([none_main],[none_attack], label='w/o defense', marker='s', color='k', s=150)
    ax[0][0].scatter([none_main],[none_main-none_attack], label='w/o defense', marker='s', color='k', s=150)
...
def plot_scatter_plot_for_paper_smaller_inFigure(input_file, target_dir, marker=False):
    with open(input_file, 'r') as f:
        data = json.load(f)
    x = {}
    y = {}
    ...
    if 'mnist' in input_file or 'cifar100' in input_file:
        ...
        axins = inset_axes(ax, width="20%", height="20%",loc='lower left',bbox_to_anchor=(0.6, 0.15, 1, 1),bbox_transform=ax.transAxes)
        for i, key in enumerate(x):
            print("key is", key)
            if key == 'none':
                continue
            ...
            axins.plot(x[key], temp, '--', linewidth=4, color=color_list[i])
            if marker:
                for j, txt in enumerate(label[key]):
                    axins.annotate(txt, (x[key][j], temp[j]))
        if 'mnist' in input_file:
            axins.set_xlim(86.2, 90.2)
            axins.set_ylim(0.0, 2.5)
        elif 'cifar100' in input_file:
            axins.set_xlim(34.9, 36.4)
            axins.set_ylim(-0.2, 0.9)
        axins.patch.set_alpha(0.4)
        mark_inset(ax, axins, loc1=1, loc2=3, fc="none", ec='k', lw=0.5) # 1(upper right) 2 (upper left) 3(lower left) 4(lower right)

    plt.tight_layout()
    if marker:
        plt.savefig(os.path.join(target_dir, input_file.split('/')[-1].split('.')[0] + '_inFig_reference.png'), dpi=200)
    else:
        plt.savefig(os.path.join(target_dir, input_file.split('/')[-1].split('.')[0] + '_inFig_nomarker.png'), dpi=200)
    plt.close()
...
if __name__ == '__main__':
    ...
    # for dataset, model in zip(['mnist', 'nuswide', 'cifar10', 'cifar20', 'cifar100'], ['mlp2', 'mlp2', 'resnet18', 'resnet18', 'resnet18']):
    # # for dataset, model in zip(['mnist', 'nuswide', 'cifar10', 'cifar20', 'cifar100'], ['mlp2', 'mlp2', 'resnet18', 'resnet18', 'resnet18']):
        # plot_scatter_plot_for_paper_smaller('{}/data_use/{}_{}.json'.format(target_dir, dataset, model), target_dir, True)
        # plot_scatter_plot_for_paper_smaller('{}/data_use/{}_{}.json'.format(target_dir, dataset, model), target_dir, False)
        plot_scatter_plot_for_paper_smaller_inFigure('{}/data_use/{}_{}.json'.format(target_dir, dataset, model), target_dir, False)
        plot_scatter_plot_for_paper_smaller_inFigure('{}/data_use/{}_{}.json'.format(target_dir, dataset, model), target_dir, True)
