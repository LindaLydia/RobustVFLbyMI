...
from models.vision import resnet18, MLP2, ActivePartyWithTrainableLayer, ActivePartyWithoutTrainableLayer, ActivePartySplitNN
from utils import *
...
class VFLSplitNN(object):
    ...
    def build_models(self, num_classes):
        if self.dataset_name == 'cifar100' or self.dataset_name == 'cifar10':
            ...
            net_b = ActivePartySplitNN(num_classes).to(self.device)
        elif self.dataset_name == 'mnist':
            net_a = self.models_dict[self.dataset_name](self.half_dim * self.half_dim * 4, num_classes).to(self.device)
            net_b = ActivePartySplitNN(num_classes).to(self.device)
    ...
    def train_batch(self, batch_data_a, batch_label, net_a, net_b, encoder, model_optimizer, criterion, gt_label, iteration):
        ...
        ######################## defense2: dp ############################
        pred_b_gradients = torch.autograd.grad(loss, pred, retain_graph=True)
        ...
        # print(pos_idx, gt_label[pos_idx])
        while gt_label[pos_idx] != torch.tensor(1):
            pos_idx += 1
        ...
        if iteration == 0:
            ...
            tf_gt_label = tf.convert_to_tensor(gt_label.cpu().numpy())
        ...
        weights_grad_b = torch.autograd.grad(pred, net_b.parameters(), grad_outputs=pred_b_gradients_clone)
        ...
        return loss.item(), train_acc, norm_leak_auc, cosine_leak_auc
    ...
    def train(self):
        ...
        test_acc = 0.0
        test_acc_topk = 0.0
        norm_scoring_auc=2.0
        cosine_scoring_auc=2.0
        for i_epoch in range(self.epochs):
            ...
            for i, (gt_data, gt_label) in enumerate(tqdm_train):
                ...
                gt_data_a = self.concate_data(gt_data)
                ...
                # ====== train batch ======
                loss, train_acc, temp_norm_scoring_auc, temp_cosine_scoring_auc = self.train_batch(gt_data_a, gt_one_hot_label, net_a, net_b, 
                                                    self.encoder, model_optimizer, criterion, gt_label, i_epoch+i)
                if i_epoch+i==0:
                    assert temp_norm_scoring_auc <= 1.0
                    assert temp_cosine_scoring_auc <= 1.0
                    norm_scoring_auc = temp_norm_scoring_auc
                    cosine_scoring_auc = temp_cosine_scoring_auc
                ...
                if (i + 1) % print_every == 0:
                    ...
                    with torch.no_grad():
                        ...
                        for gt_val_data, gt_val_label in val_loader:
                            ...
                            test_data_a = self.concate_data(gt_val_data)
        ...
        return test_acc, test_auc, norm_scoring_auc, cosine_scoring_auc
